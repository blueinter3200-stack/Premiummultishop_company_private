---
source_path: ".pi/BusinessVault/.obsidian/plugins/tasknotes/styles.css"
source_filename: "styles.css"
source_type: "text"
source_size_bytes: 612954
source_modified_at: "2026-08-12T16:19:29+09:00"
source_sha256: "b671c9e836bc0f0f21187fd9e7eea53f84d75b69a0ff8449241222006297a7c6"
...
:root {
    ...
    --cs-text-display-large: 44px;   /* reduced from 56px */
    --cs-text-display-medium: 36px;  /* reduced from 45px */
    --cs-text-display-small: 30px;   /* reduced from 36px */
    
    /* Headlines - Optimized for desktop density */
    --cs-text-headline-large: 28px;    /* reduced from 32px */
    --cs-text-headline-medium: 24px;    /* reduced from 28px */
    --cs-text-headline-small: 20px;     /* reduced from 24px */
    
    /* Titles - Compact yet readable */
    --cs-text-title-large: 18px;   /* reduced from 22px */
    --cs-text-title-medium: 14px;  /* reduced from 16px */
    --cs-text-title-small: 12px;   /* reduced from 14px */
    
    /* Body - Optimized for readability and compactness */
    --cs-text-body-large: 14px;    /* reduced from 16px */
    --cs-text-body-medium: 12px;   /* reduced from 14px */
    --cs-text-body-small: 11px;    /* slightly reduced */
    
    /* Labels - Compact component text */
    --cs-text-label-large: 12px;   /* reduced from 14px */
    --cs-text-label-medium: 11px;  /* slightly reduced */
    --cs-text-label-small: 10px;   /* more compact */
    ...
    /* Provider and calendar semantic colors */
    --tn-color-google-calendar: var(--color-blue, #4285f4);
    --tn-color-microsoft-calendar: var(--color-blue, #0078d4);
    --tn-color-time-entry: var(--color-green, #10b981);
    --tn-color-time-entry-text: var(--color-green, #059669);
    --tn-color-time-entry-bg-soft: rgba(16, 185, 129, 0.08);
    --tn-color-time-entry-bg-medium: rgba(16, 185, 129, 0.15);

    /* Dark-mode fallbacks when Obsidian theme variables are unavailable */
    --tn-color-time-entry-dark: var(--color-green, #34d399);
    --tn-color-time-entry-text-dark: var(--color-green, #6ee7b7);
    --tn-color-time-entry-bg-soft-dark: rgba(52, 211, 153, 0.1);
    --tn-color-time-entry-bg-medium-dark: rgba(52, 211, 153, 0.18);
...
.tasknotes-plugin .task-inline-preview__recurring-indicator {
    ...
    border-radius: 3px;
    padding: 1px 3px;
    transition: color 0.15s ease, filter 0.15s ease, opacity 0.15s ease, transform 0.15s ease;
    flex-shrink: 0;
    vertical-align: baseline;
    margin-right: 2px;
}
...
.tasknotes-plugin .task-inline-preview.task-inline-preview--priority-high {
    border-inline-start: 1px solid var(--color-red);
}

.tasknotes-plugin .task-inline-preview.task-inline-preview--priority-medium {
    border-inline-start: 1px solid var(--color-orange);
}
...
.tasknotes-plugin .note-card {
    ...
    border: none;
    border-radius: 0;
    border-inline-start: 1px solid transparent; /* Transparent by default */
...
.tasknotes-plugin .note-card--daily {
    border-inline-start-color: var(--tn-interactive-accent); /* Purple side accent for daily notes only */
}
...
.tasknotes-plugin .filter-bar__filter-toggle.has-active-filters::after {
  ...
  height: 8px;
  border-radius: 50%;
  background-color: var(--tn-color-recurring, var(--interactive-accent));
}
...
.tasknotes-plugin .filter-bar__main-box {
    ...
    width: min(500px, calc(100vw - 32px)); /* Default preferred width */
    max-width: min(600px, calc(100vw - 32px)); /* Maximum preferred width */
    min-width: min(400px, calc(100vw - 32px)); /* Minimum usable width */
...
.tasknotes-plugin .filter-bar__section-content {
    ...
    overflow: hidden;
    transition: opacity 0.3s cubic-bezier(0.4, 0, 0.2, 1), max-height 0.3s cubic-bezier(0.4, 0, 0.2, 1), padding 0.3s cubic-bezier(0.4, 0, 0.2, 1), border-width 0.3s cubic-bezier(0.4, 0, 0.2, 1);
}
...
.tasknotes-device-code-spinner {
	animation: tasknotes-device-code-spin 1s linear infinite;
}

@keyframes tasknotes-device-code-spin {
	from {
...
.tasknotes-plugin .date-time-picker-modal__title {
    margin: 0 0 var(--size-4-3);
    font-size: var(--font-ui-large);
    font-weight: 600;
}

.tasknotes-plugin .date-time-picker-modal__quick-actions {
    display: flex;
    flex-wrap: wrap;
    gap: var(--size-4-2);
    margin-bottom: var(--size-4-3);
...
.tasknotes-plugin .date-time-picker-modal__quick-button,
.tasknotes-plugin .date-time-picker-modal__action-button {
    border-radius: var(--radius-s);
}

.tasknotes-plugin .date-time-picker-modal__nlp-row {
    display: grid;
...
.tasknotes-plugin .date-time-picker-modal__nlp-input {
    width: 100%;
    min-width: 0;
}

.tasknotes-plugin .date-time-picker-modal__nlp-button {
    width: 32px;
    height: 32px;
    justify-content: center;
}
...
.task-selector-with-create-modal .task-card__main-row,
.task-selector-with-create-modal .task-card__content,
.task-selector-with-create-modal .task-card__title,
...
.markdown-source-view .cm-sizer > .tasknotes-relationships-widget {
    /*
     * CodeMirror editor selection autoscroll can stop when drag selection reaches
     * the non-editable widget after the editable document content.
     */
    -webkit-user-select: none;
    user-select: none;
}

.markdown-source-view .cm-sizer > .tasknotes-relationships-widget input,
.markdown-source-view .cm-sizer > .tasknotes-relationships-widget textarea,
.markdown-source-view .cm-sizer > .tasknotes-relationships-widget [contenteditable="true"] {
...
.advanced-calendar-view.advanced-calendar-view--auto-height {
    height: auto;
    min-height: 0;
    overflow: visible;
}

.advanced-calendar-view.advanced-calendar-view--auto-height .advanced-calendar-view__calendar--auto-height {
    flex: 0 0 auto;
    min-height: 0;
}
...
.fc-daygrid-event .fc-event-provider-icon,
.fc-timegrid-event .fc-event-provider-icon {
    position: absolute;
    top: 2px;
    right: 2px;
    z-index: 1;
...
.fc-daygrid-event .fc-event-provider-icon svg,
.fc-timegrid-event .fc-event-provider-icon svg {
    width: 10px;
    height: 10px;
}
...
.fc-event[data-ics-event="true"].has-related-note {
    box-shadow: inset 0 0 0 1px color-mix(in srgb, var(--text-muted) 32%, transparent);
}

.ics-related-note-indicator,
.tasknotes-plugin .ics-card__related-note-indicator {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    flex: 0 0 auto;
...
.ics-related-note-indicator svg,
.tasknotes-plugin .ics-card__related-note-indicator svg {
    width: 12px;
    height: 12px;
}

.fc-event .ics-related-note-indicator {
    margin-left: 4px;
}
...
/* Time Entry Events - recorded time, intentionally quieter than actionable tasks */
.fc-event[data-event-type="timeEntry"] {
    opacity: 0.72;
    border-radius: 4px;
    border-width: 1px;
    border-style: dashed;
    background: color-mix(in srgb, var(--tn-color-time-entry) 8%, transparent);
    border-color: color-mix(in srgb, var(--tn-color-time-entry) 60%, transparent);
}

.fc-event[data-event-type="timeEntry"] .fc-event-time,
.fc-event[data-event-type="timeEntry"] .fc-event-title {
    color: var(--tn-color-time-entry-text);
    font-weight: 400;
}

.fc-event[data-event-type="timeEntry"]:hover {
    opacity: 0.9;
    background: color-mix(in srgb, var(--tn-color-time-entry) 12%, transparent);
    box-shadow: 0 1px 3px color-mix(in srgb, var(--tn-color-time-entry) 18%, transparent);
}

/* Dark mode adjustments for time entries */
.theme-dark .fc-event[data-event-type="timeEntry"] {
    background: color-mix(in srgb, var(--tn-color-time-entry-dark) 12%, transparent);
    border-color: color-mix(in srgb, var(--tn-color-time-entry-dark) 65%, transparent);
}

.theme-dark .fc-event[data-event-type="timeEntry"] .fc-event-time,
.theme-dark .fc-event[data-event-type="timeEntry"] .fc-event-title {
    color: var(--tn-color-time-entry-text-dark);
}
...
body.is-mobile .timeblock-creation-modal input.tasknotes-theme-color-text-input,
body.is-mobile .timeblock-info-modal input.tasknotes-theme-color-text-input {
    width: auto;
}

body.is-mobile .timeblock-creation-modal input.tasknotes-theme-color-picker,
body.is-mobile .timeblock-info-modal input.tasknotes-theme-color-picker {
    width: 2.25rem;
    min-width: 2.25rem;
    max-width: 2.25rem;
}
...
.advanced-calendar-view.advanced-calendar-view--auto-height .fc {
    height: auto;
}
...
.advanced-calendar-view.advanced-calendar-view--auto-height .fc-view-harness,
.advanced-calendar-view.advanced-calendar-view--auto-height .fc-scroller {
    height: auto;
}
...
.advanced-calendar-view.advanced-calendar-view--hide-time-grid .fc-timegrid .fc-timegrid-divider {
    display: none;
}

.advanced-calendar-view.advanced-calendar-view--hide-time-grid .fc-timegrid .fc-timegrid-body {
    display: none;
}
...
/* Agenda/list mode can live in narrow sidebars; respond to container width, not viewport width. */
.advanced-calendar-view.advanced-calendar-view--list {
    overflow-x: hidden;
}

.advanced-calendar-view.advanced-calendar-view--list .fc,
.advanced-calendar-view.advanced-calendar-view--list .fc-view-harness,
.advanced-calendar-view.advanced-calendar-view--list .fc-list,
.advanced-calendar-view.advanced-calendar-view--list .fc-list-table {
    max-width: 100%;
    min-width: 0;
}

.advanced-calendar-view.advanced-calendar-view--list .fc-scroller {
    overflow-x: hidden;
}

body.is-mobile .advanced-calendar-view.advanced-calendar-view--list .fc-scroller {
...
.advanced-calendar-view.advanced-calendar-view--list .fc-list-card-content {
    box-sizing: border-box;
    padding: 4px 10px;
    vertical-align: top;
    width: 100%;
...
.advanced-calendar-view.advanced-calendar-view--list .fc-list-task-card > td:not(.fc-list-card-content) {
    display: none;
}

.advanced-calendar-view.advanced-calendar-view--list .fc-list-event.fc-list-task-card:hover,
.advanced-calendar-view.advanced-calendar-view--list .fc-list-event.fc-list-task-card:hover > .fc-list-card-content {
    background: transparent;
    background-color: transparent;
}

.advanced-calendar-view.advanced-calendar-view--list .task-card,
.advanced-calendar-view.advanced-calendar-view--list .task-card__main-row,
.advanced-calendar-view.advanced-calendar-view--list .task-card__content,
.advanced-calendar-view.advanced-calendar-view--list .task-card__title {
    min-width: 0;
    max-width: 100%;
}

.advanced-calendar-view.advanced-calendar-view--list .task-card__title-text {
    min-width: 0;
    overflow-wrap: anywhere;
}

.advanced-calendar-view.advanced-calendar-view--list .task-card__badges,
.advanced-calendar-view.advanced-calendar-view--list .task-card__recurring-indicator,
.advanced-calendar-view.advanced-calendar-view--list .task-card__reminder-indicator,
.advanced-calendar-view.advanced-calendar-view--list .task-card__details-indicator,
.advanced-calendar-view.advanced-calendar-view--list .task-card__project-indicator,
.advanced-calendar-view.advanced-calendar-view--list .task-card__chevron,
.advanced-calendar-view.advanced-calendar-view--list .task-card__blocking-toggle,
.advanced-calendar-view.advanced-calendar-view--list .task-card__blocked-toggle {
    flex: 0 0 auto;
}

@container (max-width: 480px) {
    .advanced-calendar-view.advanced-calendar-view--list .fc-header-toolbar {
        align-items: center;
        justify-content: center;
        overflow-x: hidden;
    ...
    .advanced-calendar-view.advanced-calendar-view--list .fc-toolbar-chunk {
        flex-wrap: wrap;
        justify-content: center;
    }

    .advanced-calendar-view.advanced-calendar-view--list .fc-toolbar-chunk:nth-child(2) {
        order: -1;
        flex: 0 0 100%;
    }

    .advanced-calendar-view.advanced-calendar-view--list .fc-toolbar-chunk:first-child,
    .advanced-calendar-view.advanced-calendar-view--list .fc-toolbar-chunk:nth-child(3) {
        flex: 1 1 100%;
    }

    .advanced-calendar-view.advanced-calendar-view--list .fc-toolbar-title {
        padding-inline: 0;
        text-align: center;
        white-space: normal;
    }

    .advanced-calendar-view.advanced-calendar-view--list .fc-button {
        min-width: 0;
    }
}
...
.advanced-calendar-view .fc .fc-timegrid-col.fc-day-today {
    background-color: color-mix(in srgb, var(--interactive-accent) 8%, var(--background-primary));
}

/* Multi-month (year view) styling - minimalist */
.internal-embed .advanced-calendar-view .fc-multimonth,
.markdown-embed .advanced-calendar-view .fc-multimonth {
    width: 100%;
    max-width: 100%;
}

.internal-embed .advanced-calendar-view .fc-multimonth-month,
.markdown-embed .advanced-calendar-view .fc-multimonth-month {
    min-width: 0;
}
...
.advanced-calendar-view .fc-list-day.fc-day-today .fc-list-day-cushion {
    color: var(--interactive-accent);
    font-weight: 600;
    border-bottom-color: var(--interactive-accent);
}

.hide-today-highlight .fc-list-day.fc-day-today .fc-list-day-cushion {
    color: var(--text-normal);
    font-weight: 500;
...
.advanced-calendar-view .fc-event.fc-task-event--hover-source,
.advanced-calendar-view .fc-event.fc-task-event--related-hover {
    box-shadow: inset 0 0 0 1px var(--interactive-accent), 0 2px 8px rgba(0, 0, 0, 0.14);
}

.advanced-calendar-view .fc-event.fc-task-event--hover-source {
    box-shadow: inset 0 0 0 1px var(--interactive-accent), 0 2px 8px rgba(0, 0, 0, 0.16);
}
...
.advanced-calendar-view .fc-daygrid-event,
.advanced-calendar-view .fc-timegrid-event {
    white-space: normal;
}
...
.advanced-calendar-view .fc-timegrid-all-day-events .fc-daygrid-event {
    white-space: nowrap;
}

.advanced-calendar-view .fc-timegrid-all-day-events .fc-event-main,
.advanced-calendar-view .fc-timegrid-all-day-events .fc-event-main-frame {
    min-width: 0;
}
...
.advanced-calendar-view .fc-timegrid .fc-daygrid-body .fc-event-main,
.advanced-calendar-view .fc-timegrid .fc-daygrid-body .fc-event-main-frame,
...
.advanced-calendar-view .fc-timegrid-slot-label-cushion {
    padding: 0;
}
...
.theme-dark .advanced-calendar-view .fc-more-link {
    color: var(--text-normal);
    background: var(--background-secondary);
    border: 1px solid var(--background-modifier-border);
    border-radius: 4px;
...
.theme-dark .advanced-calendar-view .fc-more-link:hover {
    color: var(--text-normal);
    background: var(--background-modifier-hover);
}
...
@media (max-width: 768px) {
    ...
    body.is-mobile .advanced-calendar-view .tn-calendar-event-touch-target {
        touch-action: manipulation;
        -webkit-touch-callout: none;
        user-select: none;
    }

    body.is-mobile .advanced-calendar-view .tn-calendar-event-touch-target .task-card {
        -webkit-touch-callout: none;
        user-select: none;
    }
    ...
    .advanced-calendar-view .fc-timegrid-event .fc-event-main {
        min-width: 0;
        padding: 0 2px;
    }

    .advanced-calendar-view .fc-timegrid-event .fc-event-main-frame {
        gap: 1px;
        min-width: 0;
    }

    .advanced-calendar-view .fc-timegrid-event .fc-event-time,
    .advanced-calendar-view .fc-timegrid-event .fc-event-title {
        overflow: hidden;
        text-overflow: clip;
        white-space: nowrap;
    }

    .advanced-calendar-view .fc-timegrid-event.fc-timegrid-event-short .fc-event-time {
        display: none;
    }

    body.is-mobile .advanced-calendar-view .fc-timegrid-event.fc-task-event,
    body.is-mobile .advanced-calendar-view .fc-timegrid-event.fc-ics-event,
    body.is-mobile .advanced-calendar-view .fc-timegrid-event[data-event-type="property-based"] {
        background-color: var(--fc-event-border-color);
    }

    body.is-mobile .advanced-calendar-view .fc-timegrid-event.fc-task-event .fc-event-main,
    body.is-mobile .advanced-calendar-view .fc-timegrid-event.fc-ics-event .fc-event-main,
    body.is-mobile .advanced-calendar-view .fc-timegrid-event[data-event-type="property-based"] .fc-event-main {
        color: var(--text-on-accent);
    }
}
...
.fc-list-task-projects {
    background-color: rgba(var(--color-blue-rgb, 13 110 253), 0.1);
    color: var(--color-blue);
}

.fc-list-task-contexts {
    background-color: rgba(var(--color-green-rgb, 25 135 84), 0.1);
    color: var(--color-green);
}

.fc-list-task-tags {
    background-color: rgba(var(--color-purple-rgb, 102 16 242), 0.1);
    color: var(--color-purple);
}

.fc-list-task-time-estimate {
    background-color: rgba(var(--color-orange-rgb, 255 159 67), 0.1);
    color: var(--color-orange);
}
...
.tasknotes-plugin .kanban-view__column--unknown-status {
    border-style: dashed;
    border-color: var(--tn-text-faint);
    background: var(--tn-bg-secondary);
}

.tasknotes-plugin .kanban-view__column-header--unknown-status {
    background: var(--tn-bg-primary);
    color: var(--tn-text-muted);
}

.tasknotes-plugin .kanban-view__column-header--unknown-status .kanban-view__column-title::after,
.tasknotes-plugin .kanban-view__column-header-cell--unknown-status .kanban-view__column-title::after {
    content: "not in settings";
    display: inline-flex;
    margin-left: var(--tn-spacing-xs);
    color: var(--tn-text-muted);
    font-size: var(--tn-font-size-xs);
...
.tasknotes-plugin .kanban-view__column-header-cell--dragover {
    background: var(--tn-interactive-hover);
    box-shadow: inset 0 0 0 1px var(--tn-interactive-accent);
}
...
.tasknotes-plugin .kanban-view__column--column-drop-target {
    background: var(--tn-interactive-hover);
    border-color: var(--tn-interactive-accent);
    transition: background-color var(--tn-transition-medium), border-color var(--tn-transition-medium), box-shadow var(--tn-transition-medium);
    box-shadow: inset 0 0 0 1px color-mix(in srgb, var(--tn-interactive-accent) 45%, transparent);
    position: relative;
}
...
.tasknotes-plugin .kanban-view__column--column-drop-target::before {
    content: "";
    position: absolute;
    inset: var(--tn-spacing-xs);
    border: 1px dashed color-mix(in srgb, var(--tn-interactive-accent) 65%, transparent);
    border-radius: var(--tn-radius-sm);
    pointer-events: none;
}
...
.tasknotes-plugin .kanban-view__column-header {
    ...
    cursor: grab;
    user-select: none;
    transition: background-color var(--tn-transition-fast), border-color var(--tn-transition-fast), box-shadow var(--tn-transition-fast), opacity var(--tn-transition-fast), transform var(--tn-transition-fast);
}
...
.tasknotes-plugin .kanban-view__drag-handle {
    ...
    justify-content: center;
    border-radius: var(--tn-radius-xs);
    transition: opacity var(--tn-transition-fast), background-color var(--tn-transition-fast), color var(--tn-transition-fast);
    flex-shrink: 0;
}
...
.tasknotes-plugin .kanban-view__drag-handle svg {
    width: 14px;
    height: 14px;
}
...
.tasknotes-plugin .kanban-view__column-icon svg {
    width: 12px;
    height: 12px;
}
...
.tasknotes-plugin .kanban-view__card-wrapper .task-card--layout-compact {
    padding: var(--tn-spacing-sm) var(--tn-spacing-md);
    border-radius: var(--tn-radius-sm);
}

.tasknotes-plugin .kanban-view__card-wrapper .task-card--layout-compact .task-card__title {
    font-size: var(--tn-font-size-md);
    line-height: 1.3;
}

.tasknotes-plugin .kanban-view__card-wrapper .task-card--layout-compact .task-card__metadata {
    gap: 2px var(--tn-spacing-sm);
}
...
.tasknotes-plugin .kanban-view__empty-cell {
    flex: 1 1 auto;
    min-height: 64px;
    margin: var(--tn-spacing-xs);
    border: 1px dashed color-mix(in srgb, var(--tn-border-color) 78%, transparent);
    border-radius: var(--tn-radius-sm);
    background: color-mix(in srgb, var(--tn-bg-secondary) 70%, transparent);
    color: var(--tn-text-faint);
    display: flex;
    align-items: center;
    justify-content: center;
...
.tasknotes-plugin .kanban-view__empty-cell-icon {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    opacity: 0.7;
...
.tasknotes-plugin .kanban-view__empty-cell-icon svg {
    width: 14px;
    height: 14px;
}
...
body.is-mobile .tasknotes-plugin .kanban-view__board {
    align-items: flex-start;
...
body.is-mobile .tasknotes-plugin .kanban-view__board--swimlanes {
    --kanban-swimlane-label-width: 72px;
    --kanban-column-width: minmax(156px, 64vw);
}

body.is-mobile .tasknotes-plugin .kanban-view__swimlane-row {
    grid-template-columns: var(--kanban-swimlane-label-width);
...
body.is-mobile .tasknotes-plugin .kanban-view__swimlane-row > .kanban-view__swimlane-label {
    width: var(--kanban-swimlane-label-width);
    min-width: var(--kanban-swimlane-label-width);
    max-width: var(--kanban-swimlane-label-width);
}
...
body.is-mobile .tasknotes-plugin .kanban-view__swimlane-title {
    max-width: 100%;
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
...
body.is-mobile .tasknotes-plugin .kanban-view__swimlane-label,
body.is-mobile .tasknotes-plugin .kanban-view__column-header-cell,
...
/* Touch drag support */
.tasknotes-plugin .kanban-view__card-wrapper[draggable="true"],
.tasknotes-plugin .kanban-view__column-header[draggable="true"],
.tasknotes-plugin .kanban-view__column-header-cell[draggable="true"] {
    touch-action: manipulation;
    -webkit-touch-callout: none;
    -webkit-user-select: none;
    user-select: none;
...
.tasknotes-plugin .kanban-view__touch-ghost {
    border-radius: var(--tn-radius-md);
    background: var(--tn-bg-primary);
    border: 2px solid var(--tn-interactive-accent);
}

.tasknotes-plugin.kanban-view__drag-image,
.tasknotes-plugin.kanban-view__floating-drag-preview {
    border: 1px solid var(--tn-interactive-accent);
    border-radius: var(--tn-radius-md);
    background: var(--tn-bg-primary);
    box-shadow: 0 8px 18px color-mix(in srgb, var(--tn-shadow-color) 24%, transparent),
        0 0 0 2px color-mix(in srgb, var(--tn-interactive-accent) 18%, transparent);
}
...
.tasknotes-plugin.kanban-view__floating-drag-preview {
    ...
    gap: var(--tn-spacing-sm);
    box-sizing: border-box;
    width: var(--tn-kanban-drag-preview-width, auto);
    min-height: 40px;
    max-height: 40px;
    padding: 0 var(--tn-spacing-md);
    pointer-events: none;
    transform: var(--tn-kanban-drag-preview-transform, translate3d(-9999px, -9999px, 0));
    z-index: var(--layer-tooltip, 10000);
    overflow: hidden;
...
.tasknotes-plugin.kanban-view__floating-drag-preview-title {
    min-width: 0;
    overflow: hidden;
    color: var(--tn-text-normal);
    font-size: var(--font-ui-small);
...
.tasknotes-plugin.kanban-view__floating-drag-preview::after {
...
.tasknotes-plugin.kanban-view__floating-drag-preview[data-drag-count]::after {
    display: block;
    opacity: 1;
}

.tasknotes-plugin .kanban-view__card--dragging {
    opacity: 0.64;
    transform: scale(0.98);
    outline: 1px dashed color-mix(in srgb, var(--tn-interactive-accent) 55%, transparent);
    outline-offset: -2px;
}

/* Drop position indicators for within-column reordering */
.tasknotes-plugin .kanban-view__card-wrapper {
    position: relative;
}
...
.tasknotes-plugin .kanban-view__card-wrapper--shift-down {
...
@media (pointer: coarse) {
    .tasknotes-plugin .kanban-view__column--dragover,
    .tasknotes-plugin .kanban-view__swimlane-column--dragover {
        border-color: var(--tn-interactive-accent);
        border-width: 2px;
...
.tasknotes-plugin .pomodoro-view__timer-header {
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
...
.tasknotes-plugin .pomodoro-view__timer-display--editable {
    cursor: text;
    border-radius: var(--tn-radius-sm);
}

.tasknotes-plugin .pomodoro-view__timer-display--editable:hover {
    color: var(--tn-interactive-accent);
}

.tasknotes-plugin .pomodoro-view__timer-display--hidden {
    display: none;
}
...
.tasknotes-plugin .pomodoro-view__timer-input:focus {
    outline: 2px solid var(--tn-interactive-accent);
    outline-offset: 2px;
}

.tasknotes-plugin .pomodoro-view__timer-input--hidden {
    display: none;
}
...
.tasknotes-plugin .pomodoro-view__session-meta {
    margin: 0;
    color: var(--tn-text-muted);
    font-size: var(--tn-font-size-xs);
    line-height: 1.25;
...
.tasknotes-plugin .pomodoro-view__task-display {
    ...
    opacity: 1;
    transform: translateY(0);
    transition: max-height var(--tn-transition-medium), opacity var(--tn-transition-medium), padding var(--tn-transition-medium), transform var(--tn-transition-medium);
}
...
.tasknotes-plugin .pomodoro-view__stat--clickable {
    ...
    padding: var(--tn-spacing-xs);
    border-radius: var(--tn-radius-sm);
    transition: background-color var(--tn-transition-fast), box-shadow var(--tn-transition-fast);
}
...
.tasknotes-plugin.pomodoro-view.pomodoro-view--very-narrow .pomodoro-view__timer-overlay .pomodoro-view__timer-input {
    font-size: 2.5rem;
    font-weight: 600;
}
...
.tasknotes-plugin.pomodoro-view.pomodoro-view--narrow .pomodoro-view__timer-overlay .pomodoro-view__timer-input {
    font-size: 3rem;
    font-weight: 600;
}
...
.tasknotes-plugin .pomodoro-stats-view__header .setting-item {
    flex: 1 1 auto;
    min-width: 0;
    border: none;
    padding: 0;
...
.tasknotes-plugin .pomodoro-stats-view__header-actions {
    display: inline-flex;
    align-items: center;
    gap: var(--tn-spacing-xs);
    flex: 0 0 auto;
...
.tasknotes-plugin .pomodoro-stats-view__bases-help {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    width: 28px;
...
.tasknotes-plugin .pomodoro-stats-view__bases-help:hover,
.tasknotes-plugin .pomodoro-stats-view__bases-help:focus-visible {
    background: var(--tn-interactive-hover);
    color: var(--tn-text-normal);
}

.tasknotes-plugin .pomodoro-stats-view__bases-help svg {
    width: 16px;
    height: 16px;
}
...
/* Obsidian 1.13+ gives plugin setting-tab roots the `setting-item` class. Its
   flex layout would place TaskNotes' toolbar and tab content side by side, so
   restore the vertical document flow used by this settings view. */
...
.tasknotes-plugin .settings-view__tab-button:focus-visible {
    outline: 2px solid var(--background-modifier-border-focus);
    outline-offset: 2px;
}
...
.tasknotes-theme-color-text-input {
    flex: 1 1 10rem;
    min-width: 0;
}

input.tasknotes-theme-color-picker {
    flex: 0 0 2.25rem;
    width: 2.25rem;
    min-width: 2.25rem;
    max-width: 2.25rem;
...
.tasknotes-calendar-info--warning {
    color: var(--text-error);
}
...
/* ===== FIELD-MANAGER.CSS ===== */
.tasknotes-plugin .field-manager {
	display: flex;
	flex-direction: column;
	gap: 1rem;
}

.tasknotes-plugin .field-manager__tabs {
	display: flex;
	gap: 0.5rem;
...
.tasknotes-plugin .field-manager__tab {
	padding: 0.5rem 1rem;
	cursor: pointer;
	border-radius: 4px;
...
.tasknotes-plugin .field-manager__tab:hover {
	background-color: var(--background-modifier-hover);
}

.tasknotes-plugin .field-manager__tab--active {
	color: var(--text-on-accent);
	background-color: var(--interactive-accent);
}

.tasknotes-plugin .field-manager__content {
	padding: 1rem 0;
}

.tasknotes-plugin .field-manager__cards {
	display: flex;
	flex-direction: column;
	gap: 0.75rem;
}

.tasknotes-plugin .field-manager__empty {
	padding: 2rem;
	color: var(--text-muted);
	text-align: center;
}

.tasknotes-plugin .field-card__type {
	padding: 0.125rem 0.5rem;
	font-size: 0.75rem;
	background: var(--background-modifier-border);
	border-radius: 3px;
...
.tasknotes-plugin .field-card__type--core {
	color: var(--text-on-accent);
	background: var(--interactive-accent);
}

.tasknotes-plugin .field-card__type--user {
	color: var(--text-on-accent);
	background: var(--color-purple);
}

.tasknotes-plugin .field-card__type--dependency {
	color: var(--text-on-accent);
	background: var(--color-orange);
}

.tasknotes-plugin .field-card__type--organization {
	color: var(--text-on-accent);
	background: var(--color-green);
}
...
/* Static utility classes migrated from literal inline styles. */
/* Generated by lint cleanup; edit the source call sites and rebuild when changing these styles. */
